...
while True:
    ...
    if image_provider_choice == "1":
        ...
        break
    elif image_provider_choice == "2":
        IMAGE_PROVIDER = "Huggingface"
        break
    console.print("[warning]Invalid choice. Please try again.[/warning]")

console.print(Panel("[bold]Enter Image Generation model name[/bold]", title="Image Model Configuration"))
image_model_name = Prompt.ask("Image Generation model name", default={
    "Gemini": "gemini-2.0-flash-exp-image-generation",
    "Huggingface": "black-forest-labs/FLUX.1-dev"
}.get(IMAGE_PROVIDER, "default-image-model"))

if IMAGE_PROVIDER == "Gemini":
    console.print(Panel(f"[bold]Enter Gemini API Key (Leaving it blank sets Image Generation: off)[/bold]", title="Image Generation Model API Key Input"))
    image_api_key = get_masked_input(f"Enter Gemini API Key (input will be masked): ")
...
def change_model_provider():
    ...
    while True:
        ...
        console.print("[warning]Invalid choice. Please try again.[/warning]")
    console.print(Panel("[bold]Enter Image Generation model name[/bold]", title="Image Model Configuration"))
    new_image_model_name = Prompt.ask("Image Generation model name", default={
        "Gemini": "gemini-2.0-flash-exp-image-generation",
        "Huggingface": "black-forest-labs/FLUX.1-dev"
    }.get(IMAGE_PROVIDER, "default-image-model"))
    if IMAGE_PROVIDER == "Gemini":
    ...
    web_app_generator = create_web_app_generator(client)
    code_generator = create_code_generator(client)
    shell_agent = create_shell_agent(client)
...
def image(client, IMAGE_MODEL, image_api_key):
    ...
    if not image_api_key or not image_api_key.strip():
        console.print(Panel(f"[warning]No API key found for {IMAGE_PROVIDER} image generation[/warning]", style="bold yellow"))
        image_api_key = get_masked_input(f"Enter {IMAGE_PROVIDER} API Key (input will be masked, press Enter to skip): ")
        if not image_api_key.strip():
            console.print(Panel("[warning]Cannot generate image without connecting to image provider. Exiting Image Generation Mode.[/warning]", style="bold red"))
            return
    
    while True:
        text_prompt = Prompt.ask(Text(f"Enter the text prompt for the image (or 'quit' to exit, 'editor' for Image Editor Mode)", style="green"))
        if text_prompt.lower() == 'quit':
            console.print(Panel("**Exited Image Generation Mode**", style="bold red"))
            break
        elif text_prompt.lower() == 'editor':
            if IMAGE_PROVIDER != "Gemini":
                console.print(Panel("[warning]Image Editor Mode is only supported with the Gemini provider.[/warning]", style="bold yellow"))
                continue
            console.print(Panel("[bold]Image Editor Mode Activated[/bold]", title="Image Editor Mode", style="bold blue"))
            while True:
                edit_input = Prompt.ask(Text(f"Enter the image path to edit (or 'back' to return to Image Generation Mode)", style="green"))
                if edit_input.lower() == 'back':
                    console.print(Panel("**Returned to Image Generation Mode**", style="bold blue"))
                    break
                try:
                    input_image = Image.open(edit_input)
                except Exception as e:
                    console.print(Panel(f"Error loading image: {e}", style="bold red"))
                    continue
                
                edit_prompt = Prompt.ask(Text(f"Enter the edit instruction (e.g., 'change the hat color to red')", style="green"))
                while True:
                    OUTPUT_FILE_NAME = Prompt.ask("Enter the output file name for the edited image (e.g., edited_image.png)", default="edited_image.png")
                ...
                spinner = Spinner("dots", text=Text("Editing image with Gemini", style="green"))
                try:
                    gemini_client = genai.Client(api_key=image_api_key)
                except Exception as e:
                    console.print(Panel(f"Error initializing Gemini client: {e}", style="bold red"))
                    continue
                with Live(spinner, console=console, refresh_per_second=10) as live:
                    try:
                        ...
                        console.print(Panel(f"Error editing image with Gemini: {e}", style="bold red"))
                        continue
                ...
                try:
                    ...
                    console.print(Panel(f"Edited image saved to: [success]{output_path}[/success]", title="Image Saved"))
                    console.print(Panel(f"Image editing time: [bold]{elapsed_time:.2f} seconds[/bold]", title="Editing Time"))
                    if operating_system == "Termux":
                        handle_termux_file_options(output_path)
                except Exception as e:
                    console.print(Panel(f"Error saving edited image: {e}", style="bold red"))
                if Prompt.ask("Do you want to edit another image? (yes/no)", default="no").lower() != "yes":
                    console.print(Panel("**Returned to Image Generation Mode**", style="bold blue"))
                    break
            continue
        ...
        start_time = time.time()
        spinner = Spinner("dots", text=Text(f"Generating image with {IMAGE_PROVIDER}", style="green"))
        ...
        if IMAGE_PROVIDER == "Gemini":
            ...
            with Live(spinner, console=console, refresh_per_second=10) as live:
                try:
                    ...
                    console.print(Panel(f"Error generating image with Gemini: {e}", style="bold red"))
                    continue
            ...
            try:
                ...
                console.print(Panel(f"Error initializing Huggingface image client: {e}", style="bold red"))
                continue
            with Live(spinner, console=console, refresh_per_second=10) as live:
                try:
                    image = image_client.text_to_image(text_prompt)
                except Exception as e:
                    console.print(Panel(f"Error generating image with Huggingface: {e}", style="bold red"))
                    continue
